...
def create_order(data: dict) -> str:
    """Create a buy or sell order for one share of a stock
    and fullfill it if possible."""
    # These are test values
    # We will have a function that returns this data using API call parameters
    order = data

    # If the current transaction is a buy order, select all active sell orders by price then by time-posted (desc)
    valid_transactions = None
    ...
    else: # If the current order is a sell order, select all active buy orders by price then by time-posted (asc)
        valid_transactions = (
        supabase.table("active_buy_sell")
            .select(
                "Id",
    ...
    # Market is open and a valid transactions are available
    if not valid_transactions:
        ...
        # The message will change depending of the type of order (buy or sell)
        message = "No valid "
        if order["buy_or_sell"]:
            message += "sells"
        else:
    ...
    # Variables to identify who is the buyer and seller
    buyer = None
    seller = None
    # Identify is the order created by the user is a buy or sell order
    if order["buy_or_sell"]:
        buyer = order
        # Get sell order closest to the buy price
        seller = valid_transactions.pop()
    else:
        seller = order
        # Get buy order closest to the buy price
        buyer = valid_transactions.pop()

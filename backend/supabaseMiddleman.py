...
def fetch_user_data(userID: str, portfolioOrProfiles: Literal['portfolio', 'profiles']) -> dict:
    ...
    if portfolioOrProfiles not in ['Portfolio', 'profiles']:
        raise ValueError ("portfolioOrProfiles must be either 'Portfolio' or 'profiles'.")
    
    return supabase.table(f"{portfolioOrProfiles}").select("*").eq("userId", userID).execute().data

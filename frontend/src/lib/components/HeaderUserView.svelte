<script lang="ts">
	...
	function checkDropdownFocus(event: Event) {
		if (dropdown != undefined) {
			if (dropdown.contains(event.target) == true && dropdownButton.contains(event.target) == false) {
				isDropdownVisible = true;
			} else if (dropdown.contains(event.target) == false && dropdownButton.contains(event.target) == true && isDropdownVisible == true) {
				isDropdownVisible = true;
			} else {
...
<svelte:window on:click={checkDropdownFocus}/>
